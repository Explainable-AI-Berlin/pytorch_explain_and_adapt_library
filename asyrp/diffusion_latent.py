...
from asyrp.configs.paths_config import DATASET_PATHS, MODEL_PATHS
...
class Asyrp(object):
    ...
    def run_training(self):
        ...
        cosine, clip_loss_func = self.set_t_edit_t_addnoise(
        ...
        if self.args.load_random_noise:
            # get Random noise xT
            img_lat_pairs_dic = self.random_noise_pairs(
                model,
            ...
            # get Real image xT
            img_lat_pairs_dic = self.precompute_pairs(
                model, self.args.save_precomputed_images
            )
                        ...
                        with tqdm(
                            ...
                            for t_it, (i, j) in enumerate(
                                ...
                                progress_bar.set_description(
                                    f"{step}-{it_out}: loss_clr: {loss_clr:.3f} loss_l1: {loss_l1:.3f} loss_id: {loss_id:.3f} loss_clip:{loss_clip} loss: {loss:.3f} "
                                )
    ...
    def save_image(
            ...
            if save_x_origin:
                ...
                for it, (i, j) in enumerate(
                    ...
                    x, x0_t, _, _ = denoising_step(
                        ...
                        eta=1.0
                        if (
                            self.args.origin_process_addnoise and t[0] < self.t_addnoise
                        )
                        else 0.0,
                    )
                    progress_bar.update(1)
                ...
                for hs_coeff_tuple in hs_coeff:
                    ...
                    for it, (i, j) in enumerate(
                        ...
                        x, x0_t, delta_h, _ = denoising_step(
                            ...
                            if (self.args.ignore_timesteps and self.args.train_delta_h)
                        ...
                        if save_process_delta_h:
                            ...
                            output = (output + 1) * 0.5
                            grid = tvu.make_grid(
                                output, nrow=self.args.bs_train, padding=1
                            )
                            tvu.save_image(
                                grid,
                                os.path.join(
        ...
        time_e = time.time()
        print(
            f"{time_e - time_s} seconds, {file_name}_ngen{self.args.n_train_step}.png is saved"
        )
    ...
    def run_test(self):
        ...
        # Set self.t_edit & self.t_addnoise & return cosine similarity of attribute
        cosine = self.set_t_edit_t_addnoise(
            LPIPS_th=self.args.lpips_edit_th,
            LPIPS_addnoise_th=self.args.lpips_addnoise_th,
        ...
        if self.args.num_mean_of_delta_hs:
            ...
            if os.path.isfile(
                f"checkpoint_latent/{exp_id}_{self.args.n_test_step}_{self.args.num_mean_of_delta_hs}.pth"
            ):
        ...
        scaling_factor = (
            self.args.n_train_step / self.args.n_test_step * self.args.hs_coeff_delta_h
        )
        ...
        if self.args.multiple_attr:
            ...
            hs_coeff = [1.0 * self.args.hs_coeff_origin_h] + [
        ...
        if self.args.do_train:
            for step, (x0, _, x_lat) in enumerate(img_lat_pairs_dic["train"]):
                ...
                self.save_image(
                    ...
                    delta_h_dict=delta_h_dict,
                    folder_dir=self.args.test_image_folder,
                    get_delta_hs=self.args.num_mean_of_delta_hs,
                    save_process_origin=self.args.save_process_origin,
                    save_process_delta_h=self.args.save_process_delta_h,
                    file_name=f"train_{step}_{self.args.n_iter - 1}",
                    hs_coeff=hs_coeff,
                )
                ...
                if step == self.args.num_mean_of_delta_hs - 1:
                    ...
                    print(
                        f"Dict: checkpoint_latent/{exp_id}_{self.args.n_test_step}_{self.args.num_mean_of_delta_hs}.pth is saved."
                    )
        ...
        for mode in ["train", "test"]:
            ...
            if os.path.exists(pairs_path) and not self.args.re_precompute:
                ...
                for exist_precompute_num in reversed(
                    range(
                        self.args.n_train_img
                        if mode == "train"
                        else self.args.n_test_img
                    )
                ):
                ...
                loader_dic = get_dataloader(
                    train_dataset,
                    test_dataset,
                    bs_train=1,  # self.args.bs_train,
                    num_workers=self.config.data.num_workers,
                    shuffle=self.args.shuffle_train_dataloader,
                )
                loader = loader_dic[mode]

                if self.args.save_process_origin:
                    save_process_folder = os.path.join(
                        self.args.image_folder, f"inversion_process"
                    )
                    if not os.path.exists(save_process_folder):
                        os.makedirs(save_process_folder)

            for step, img in enumerate(loader):
                if (mode == "train" and step == self.args.n_train_img) or (
                    mode == "test" and step == self.args.n_test_img
                ):
                    break
                if exist_num != 0:
        ...
        if saved_noise:
            for mode in ["train", "test"]:
                ...
                while True:
                    ...
                    if (mode == "train" and step == self.args.n_train_img - 1) or (
                        mode == "test" and step == self.args.n_test_img - 1
                    ):
                        break
                    step += 1
    ...
    def compute_lpips_distance(self):
        ...
        for step, img in enumerate(loader):
            ...
            if save_imgs:
                tvu.save_image(
                    (x0 + 1) * 0.5,
                    os.path.join(self.args.image_folder, f"LPIPS_{step}_0_orig.png"),
                )
                    ...
                    for it, (i, j) in enumerate(zip((seq_inv_next[1:]), (seq_inv[1:]))):
                        ...
                        if save_imgs:
                            tvu.save_image(
                                (x + 1) * 0.5,
                                os.path.join(
                                    self.args.image_folder, f"LPIPS_{step}_{j}.png"
                                ),
                            )
                            tvu.save_image(
                                (x0_t + 1) * 0.5,
                                os.path.join(
                                    self.args.image_folder, f"X0_t_LPIPS_{step}_{j}.png"
                                ),
                            )
                        progress_bar.update(1)
    ...
    def set_t_edit_t_addnoise(
        self, LPIPS_th=0.33, LPIPS_addnoise_th=0.1, return_clip_loss=False
    ):
