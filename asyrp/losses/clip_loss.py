...
from asyrp.utils.text_templates import (
    imagenet_templates,
    part_templates,
...
class CLIPLoss(torch.nn.Module):
    def __init__(
        ...
        self.preprocess = transforms.Compose(
            ...
            + clip_preprocess.transforms[  # Un-normalize from [-1.0, 1.0] (GAN output) to [0, 1].
                :2
            ]
        ...
        self.preprocess_cnn = transforms.Compose(
            [transforms.Normalize(mean=[-1.0, -1.0, -1.0], std=[2.0, 2.0, 2.0])]
            + preprocess_cnn.transforms[  # Un-normalize from [-1.0, 1.0] (GAN output) to [0, 1].
                :2
            ]
            + preprocess_cnn.transforms[4:]  # to match CLIP input scale assumptions
        )  # + skip convert PIL to tensor
    ...
    def distance_with_templates(
        self, img: torch.Tensor, class_str: str, templates=imagenet_templates
    ) -> torch.Tensor:
    ...
    def get_text_features(
        self, class_str: str, templates=imagenet_templates, norm: bool = True
    ) -> torch.Tensor:
    ...
    def compute_img2img_direction(
        self, source_images: torch.Tensor, target_images: list
    ) -> torch.Tensor:
        with torch.no_grad():
            ...
            for target_img in target_images:
                preprocessed = (
                    self.clip_preprocess(Image.open(target_img))
                    .unsqueeze(0)
                    .to(self.device)
                )
    ...
    def set_text_features(self, source_class: str, target_class: str) -> None:
        source_features = self.get_text_features(source_class).mean(
            axis=0, keepdim=True
        )
        self.src_text_features = source_features / source_features.norm(
            dim=-1, keepdim=True
        )

        target_features = self.get_text_features(target_class).mean(
            axis=0, keepdim=True
        )
        self.target_text_features = target_features / target_features.norm(
            dim=-1, keepdim=True
        )
    ...
    def clip_angle_loss(
        ...
        cos_img_angle = torch.clamp(
            target_img_features @ src_img_features, min=-1.0, max=1.0
        )
    ...
    def compose_text_with_templates(
        self, text: str, templates=imagenet_templates
    ) -> list:
    ...
    def random_patch_centers(self, img_shape, num_patches, size):
        ...
        patch_centers = np.concatenate(
            [
                np.random.randint(
                    half_size, width - half_size, size=(batch_size * num_patches, 1)
                ),
                np.random.randint(
                    half_size, height - half_size, size=(batch_size * num_patches, 1)
                ),
            ],
            axis=1,
        )
    ...
    def patch_scores(
        self, img: torch.Tensor, class_str: str, patch_centers, patch_size: int
    ) -> torch.Tensor:
    ...
    def clip_patch_similarity(
        ...
        patch_centers = self.random_patch_centers(
            src_img.shape, 4, patch_size
        )  # TODO remove magic number

        src_scores = self.patch_scores(src_img, source_class, patch_centers, patch_size)
        target_scores = self.patch_scores(
            target_img, target_class, patch_centers, patch_size
        )
    ...
    def patch_directional_loss(
        ...
        if self.patch_text_directions is None:
            src_part_classes = self.compose_text_with_templates(
                source_class, part_templates
            )
            target_part_classes = self.compose_text_with_templates(
                target_class, part_templates
            )
            ...
            self.patch_text_directions = torch.cat(
                [
                    self.compute_text_direction(pair[0], pair[1])
                    for pair in parts_classes
                ],
                dim=0,
            )
        ...
        cosine_dists = 1.0 - self.patch_direction_loss(
            edit_direction.unsqueeze(1), self.patch_text_directions.unsqueeze(0)
        )

        patch_class_scores = cosine_dists * (
            edit_direction @ self.patch_text_directions.T
        ).softmax(dim=-1)

        return patch_class_scores.mean()

    def cnn_feature_loss(
        self, src_img: torch.Tensor, target_img: torch.Tensor
    ) -> torch.Tensor:
    ...
    def forward(
        ...
        if self.lambda_global:
            clip_loss += self.lambda_global * self.global_clip_loss(
                target_img, [f"a {target_class}"]
            )

        if self.lambda_patch:
            clip_loss += self.lambda_patch * self.patch_directional_loss(
                src_img, source_class, target_img, target_class
            )
        ...
        if self.lambda_manifold:
            clip_loss += self.lambda_manifold * self.clip_angle_loss(
                src_img, source_class, target_img, target_class
            )

        if self.lambda_texture and (texture_image is not None):
            clip_loss += self.lambda_texture * self.cnn_feature_loss(
                texture_image, target_img
            )
