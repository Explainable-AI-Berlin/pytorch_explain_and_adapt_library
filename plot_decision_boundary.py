...
def main(dataset, file_suffix, original_model=None, corrected_model=None, save_plots_to="./plots"):
    pathlib.Path(save_plots_to).mkdir(exist_ok=True, parents=True)
    if original_model is not None:
        dataset.visualize_decision_boundary(original_model, 32, device, os.path.join(save_plots_to, f"decision_boundary_original{file_suffix}.png"))
    if corrected_model is not None:
        dataset.visualize_decision_boundary(corrected_model, 32, device, os.path.join(save_plots_to, f"decision_boundary_corrected{file_suffix}.png"))
...
if __name__ == '__main__':
    ...
    if original_model is None and corrected_model is None:
        raise ValueError("Specify at least one of --model-corrected or --model-uncorrected")

    main(dataset, args.file_suffix, original_model=original_model, corrected_model=corrected_model, save_plots_to=args.save_to)