apptainer run --nv python_container.sif python3 generate_dataset.py \
--data_config '<PEAL_BASE>/configs/data/CopyrightTag_celeba.yaml'
apptainer run --nv python_container.sif python3 train_model.py \
...
apptainer run --nv python_container.sif python3 train_diffusion_model.py \
