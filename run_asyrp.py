...
def parse_args_and_config():
    ...
    parser.add_argument(
        ...
        type=str,
        default="script.sh",
        help="copy the script this file",
    )
    ...
    parser.add_argument(
        "--user_defined_t_edit",
        type=int,
        help="if you do not use it, t_edit will be set automatically",
    )
    parser.add_argument(
        "--user_defined_t_addnoise",
        type=int,
        help="if you do not use it, t_addnoise will be set automatically",
    )
    parser.add_argument(
        "--lpips_edit_th",
        type=float,
        default=0.33,
        help="we use lpips_edit_th to get t_edit",
    )
    parser.add_argument(
        "--lpips_addnoise_th",
        type=float,
        default=0.1,
        help="we use lpips_addnoise_th to get t_addnoise",
    )

    parser.add_argument(
        "--add_noise_from_xt", action="store_true", help="add_noise_from_xt"
    )
    parser.add_argument(
        "--origin_process_addnoise", action="store_true", help="origin_process_addnoise"
    )

    # Training Mode
    parser.add_argument("--run_train", action="store_true", help="run_train")
    parser.add_argument(
        "--train_delta_block", action="store_true", help="train Delta_block"
    )
    parser.add_argument("--train_delta_h", action="store_true", help="train Delta_h")
    parser.add_argument(
        "--image_space_noise_optim",
        action="store_true",
        help="train image_space_noise_optim",
    )
    parser.add_argument(
        "--image_space_noise_optim_delta_block",
        action="store_true",
        help="image_space_noise_optim_delta_block",
    )
    parser.add_argument(
        "--just_precompute", action="store_true", help="just_precompute"
    )
    parser.add_argument(
        "--ignore_timesteps",
        action="store_true",
        default=False,
        help="train without timesteps",
    )
    parser.add_argument(
        "--use_id_loss", action="store_true", default=False, help="train with id loss"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--re_precompute", action="store_true", default=False, help="re-precompute"
    )
    parser.add_argument(
        "--save_checkpoint_only_last_iter",
        action="store_true",
        default=False,
        help="carefully",
    )
    ...
    parser.add_argument(
        "--start_iter_when_you_use_pretrained",
        type=int,
        default=0,
        help="start_iter_when_you_use_pretrained",
    )
    ...
    parser.add_argument(
        "--save_x0", action="store_true", help="save x0_tensor (original image)"
    )
    parser.add_argument(
        "--save_x_origin",
        action="store_true",
        help="save x_origin (original DDIM processing)",
    )

    parser.add_argument(
        "--custom_train_dataset_dir", type=str, default="./custom/train"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="max delta for evaluating the generative process",
    )
    parser.add_argument(
        "--min_delta",
        type=float,
        default=0.0,
        help="min delta for evaluating the generative process",
    )
    parser.add_argument(
        "--num_delta",
        type=int,
        default=5,
        help="num of delta for evaluating the generative process",
    )
    ...
    parser.add_argument(
        "--target_image_id",
        type=str,
        help="Sampling only one image which is target_image_id",
    )
    parser.add_argument(
        "--start_image_id", type=int, default=0, help="Sampling after start_image_id"
    )

    parser.add_argument(
        "--save_process_origin", action="store_true", help="save_origin_process"
    )
    parser.add_argument(
        "--save_process_delta_h", action="store_true", help="save_delta_h_process"
    )

    parser.add_argument(
        "--num_mean_of_delta_hs",
        type=int,
        default=0,
        help="Get mean of delta_h from num of data",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="multiple attr for evaluating the generative process",
    )
    parser.add_argument(
        "--multiple_hs_coeff", type=str, default="", help="multiple coeffs"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="manually choose the name of chekcpoint",
    )
    parser.add_argument(
        "--choose_checkpoint_num",
        type=str,
        default="",
        help="if model is saved during an iteration, you can choose the number of chekcpoint of the model. This is for training from random noise",
    )
    ...
    parser.add_argument(
        "--style_transfer_style_from_train_images", default=False, action="store_true"
    )
    ...
    parser.add_argument(
        "--classification_results_file_name", type=str, default="classification_results"
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="if false, l1 loss is with origin_predicted_x0",
    )

    parser.add_argument(
        "--pass_origin_and_save_real_image", default=False, action="store_true"
    )
    parser.add_argument(
        "--style_transfer_use_mean",
        default=False,
        action="store_true",
        help="if false, use adain (default)",
    )
    parser.add_argument(
        "--analysis", default=False, action="store_true", help="dont save"
    )
    ...
    parser.add_argument(
        "--LPIPS_addnoise_th", type=float, default=0.1, help="LPIPS_addnoise_th"
    )
    parser.add_argument(
        "--n_test_pretrained_inv_step",
        type=int,
        default=40,
        help="# of steps during generative pross for inversion",
    )
    ...
    parser.add_argument(
        "--save_train_image_step",
        type=int,
        default=4,
        help="Wheter to save training results during CLIP fineuning",
    )
    parser.add_argument(
        "--save_train_image_iter",
        type=int,
        default=1,
        help="Wheter to save training results during CLIP fineuning",
    )
    ...
    parser.add_argument(
        "--get_h_num", type=int, default=0, help="Training batch size during Latent CLR"
    )
    parser.add_argument(
        "--model_ratio",
        type=float,
    ...
    parser.add_argument(
        "--content_dir",
        type=str,
        default="./source_images/content",
        help="Path to the content images",
    )
    parser.add_argument(
        "--style_dir",
        type=str,
        default="./source_images/style",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Number of steps for generating images",
    )
    parser.add_argument(
        "--content_replace_step",
        type=int,
        default=50,
        help="Number of steps for replacing content images",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="dt lambda coefficient for sampling calibration",
    )
    ...
    parser.add_argument(
        "--clr_loss_w", type=int, default=3, help="Weights of CLIP loss"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2e-6,
        help="Initial learning rate for latent clr",
    )
    parser.add_argument(
        "--lr_clip_lat_opt",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Whether to increase the learning rate",
    )
    ...
    parser.add_argument(
        "--interpolation_step", type=int, default=4, help="Using for debug"
    )
    parser.add_argument("--maintain_min", type=int, default=50, help="")

    parser.add_argument(
        "--get_SNR", action="store_true", default=False, help="Whether to get SNR"
    )
    ...
    args.test_image_folder = os.path.join(
        args.exp, "test_images", str(args.n_test_step)
    )
...
def asyrp_main(args=None):
    ...
    if torch.cuda.is_available():
        assert (
            args.bs_train % torch.cuda.device_count() == 0
        ), f"Number of GPUs ({torch.cuda.device_count()}) must be a multiple of batch size ({args.bs_train})"
