...
class FIDMachine:
    def __init__(self, dims=2048, device="cpu", num_samples=500):
        self.dims = dims
        self.device = device
    ...
    def compute_fid(self):
        ...
        return self.calculate_frechet_distance(cl_mu, cl_sigma, cf_mu, cf_sigma).item()

    def save_chunk_feature(self, output_path, exp_name, chunk, num_chunks):
        os.makedirs(
            osp.join(output_path, "Results", exp_name, "chunk-data"), exist_ok=True
        )
        output_path_clean = osp.join(
            output_path,
            "Results",
            exp_name,
            "chunk-data",
