...
def arguments():
    ...
    parser.add_argument("--celeba-path", required=True, type=str, help="CelebA path")
    parser.add_argument(
        "--dataset", default="CelebA", type=str, help="Is CelebAHQ dataset"
    )
    parser.add_argument("--query-label", required=True, type=int, help="CelebA path")
    ...
    def switch(self, partition):
        ...
        for CL, CF in itertools.product(["CC", "IC"], LCF):
            self.images += [
                (CL, CF, I)
                for I in os.listdir(
                    osp.join(self.path, "Results", self.exp_name, CL, CF, "CF")
                )
            ]
...
def get_correlations(path, query_label, HQ=False):
    ...
    for i in range(40):
        corrs[i] = np.corrcoef(
            df.iloc[:, query_label + c].to_numpy(), df.iloc[:, i + c].to_numpy()
        )[0, 1]
...
class CelebaOracle:
    def __init__(self, weights_path, device):
        self.oracle = OracleMetrics(weights_path=weights_path, device=device)
        self.oracle.eval()
...
def compute_CorrMetric(
    ...
    if show:
        if sorted is None:
            ...
            plt.bar(
                np.arange(len(our_corrs))[:top] + 0.15,
                our_corrs[sorted][:top],
                width=0.3,
                label="Metric",
            )
            plt.xticks(
                np.arange(len(our_corrs))[:top],
                [our_corrs[i] for i in sorted][:top],
                rotation=90,
            )
...
def plot_bar(data, labs, top, sorted, labels):
    ...
    if sorted is None:
        ...
        plt.xticks(
            x_base[:top], [labels[i] for i in sorted][:top], rotation=r, fontsize=f
        )
...
if __name__ == "__main__":
    ...
    if args.dataset == "CelebA":
        oracle = CelebaOracle(weights_path=args.oracle_path, device=device)
    else:
        oracle = CelebaHQOracle(weights_path=args.oracle_path, device=device)

    corrs, labels = get_correlations(
        args.celeba_path, args.query_label, args.dataset == "CelebAHQ"
    )
