...
class PGD_L2(Attacker):
    ...
    def attack(
        ...
        if num_noise_vectors == 1:
            ...
            if no_grad:
                with torch.no_grad():
                    return self._attack_mutlinoise_no_grad(
                        model, inputs, labels, noise, num_noise_vectors, targeted
                    )
            else:
                return self._attack_mutlinoise(
                    model, inputs, labels, noise, num_noise_vectors, targeted
                )
    ...
    def _attack_mutlinoise(
        ...
        multiplier = 1 if targeted else -1
        delta = torch.zeros(
            (len(labels), *inputs.shape[1:]), requires_grad=True, device=self.device
        )
        ...
        for i in range(self.steps):
            ...
            average_softmax = (
                softmax.reshape(-1, num_noise_vectors, logits.shape[-1])
                .mean(1, keepdim=True)
                .squeeze(1)
            )
...
class DDN(Attacker):
    ...
    def _attack(
        ...
        delta = torch.zeros_like(inputs, requires_grad=True)
        norm = torch.full(
            (batch_size,), self.init_norm, device=self.device, dtype=torch.float
        )
        ...
        for i in range(self.steps):
            ...
            delta.data.mul_(
                (norm / delta.data.view(batch_size, -1).norm(2, 1)).view(-1, 1, 1, 1)
            )
            delta.data.add_(inputs)
            if self.quantize:
    ...
    def _attack_mutlinoise(
        ...
        adv_found = torch.zeros(
            inputs[::num_noise_vectors].size(0), dtype=torch.uint8, device=self.device
        )
