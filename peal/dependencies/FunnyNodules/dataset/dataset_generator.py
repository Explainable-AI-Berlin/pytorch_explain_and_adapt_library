...
def generate_nodule(size_px=224, roundness=1, spiculation=1, edge_sharpness=1, size_attr=3, intensity=3,
    ...
    attribute_ROIs = {"roundness": scale_lin(roi_roundness), "spiculation": scale_nn(roi_spiculation),
    ...
    canvas_rois = {}
    for k, roi in attribute_ROIs.items():
        if roi.dtype == np.float32 or roi.dtype == np.float64:
            canvas_rois[k] = np.zeros((size_px, size_px), dtype=np.float32)
        else:
            canvas_rois[k] = np.zeros((size_px, size_px), dtype=np.uint8)
