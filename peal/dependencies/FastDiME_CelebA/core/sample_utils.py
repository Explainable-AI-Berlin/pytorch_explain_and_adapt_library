...
@torch.enable_grad()
def dist_cond_fn(x_tau, z_t, x_t, alpha_t, l1_loss, l2_loss, l_perc, scale_grads):
    """
...
def get_FastDiME_iterative_sampling(use_sampling=False):
    ...
    def p_sample_loop(
        ...
        for jdx, i in enumerate(indices):
            ...
            grads += (
                class_grad_fn(x_t=x_t, **class_grad_kwargs) / alpha_t
                if scale_grads
                else class_grad_fn(x_t=x_t, **class_grad_kwargs)
            )
            ...
            if (
                fast_dime_kwargs["self_optimized_masking"]
                and jdx >= fast_dime_kwargs["warmup_step"]
            ):
