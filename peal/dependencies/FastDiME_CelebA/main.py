...
def merge_and_compute_overall_metrics(args, device):
    ...
    for chunk in range(args.num_chunks):
        ...
        for k in ["bkl", "l_1", "FVA", "MNAC"]:
            ...
            summary["class-cor"][k] += (
                chunk_summary["class-cor"][k] * chunk_summary["class-cor"]["n"]
            )
            summary["class-inc"][k] += (
                chunk_summary["class-inc"][k] * chunk_summary["class-inc"]["n"]
            )

            summary["cf-cor"][k] += (
                chunk_summary["cf-cor"][k] * chunk_summary["cf-cor"]["n"]
            )
            summary["cf-inc"][k] += (
                chunk_summary["cf-inc"][k] * chunk_summary["cf-inc"]["n"]
            )

            summary["class-cor"]["cf-cor"][k] += (
                chunk_summary["class-cor"]["cf-cor"][k]
                * chunk_summary["class-cor"]["cf-cor"]["n"]
            )
            summary["class-cor"]["cf-inc"][k] += (
                chunk_summary["class-cor"]["cf-inc"][k]
                * chunk_summary["class-cor"]["cf-inc"]["n"]
            )
            summary["class-inc"]["cf-cor"][k] += (
                chunk_summary["class-inc"]["cf-cor"][k]
                * chunk_summary["class-inc"]["cf-cor"]["n"]
            )
            summary["class-inc"]["cf-inc"][k] += (
                chunk_summary["class-inc"]["cf-inc"][k]
                * chunk_summary["class-inc"]["cf-inc"]["n"]
            )
    ...
    for k in ["bkl", "l_1", "FVA", "MNAC"]:
        ...
        summary["class-cor"][k] = div(
            summary["class-cor"][k], summary["class-cor"]["n"]
        )
        summary["class-inc"][k] = div(
            summary["class-inc"][k], summary["class-inc"]["n"]
        )

        summary["cf-cor"][k] = div(summary["cf-cor"][k], summary["cf-cor"]["n"])
        summary["cf-inc"][k] = div(summary["cf-inc"][k], summary["cf-inc"]["n"])

        summary["class-cor"]["cf-cor"][k] = div(
            summary["class-cor"]["cf-cor"][k], summary["class-cor"]["cf-cor"]["n"]
        )
        summary["class-cor"]["cf-inc"][k] = div(
            summary["class-cor"]["cf-inc"][k], summary["class-cor"]["cf-inc"]["n"]
        )
        summary["class-inc"]["cf-cor"][k] = div(
            summary["class-inc"]["cf-cor"][k], summary["class-inc"]["cf-cor"]["n"]
        )
        summary["class-inc"]["cf-inc"][k] = div(
            summary["class-inc"]["cf-inc"][k], summary["class-inc"]["cf-inc"]["n"]
        )
    ...
    with open(
        osp.join(args.output_path, "Results", args.exp_name, "summary.yaml"), "w"
    ) as f:
        yaml.dump(summary, f)
...
def main(args=None):
    ...
    if args.method == "fastdime":
        sample_fn = get_FastDiME_iterative_sampling(
            use_sampling=args.use_sampling_on_x_t
        )
        self_optimized_masking = True
        ...
        sample_fn_step_2 = get_FastDiME_iterative_sampling(
            use_sampling=args.use_sampling_on_x_t
        )
    ...
    save_imgs = (
        ImageSaver(args.output_path, args.exp_name, extention=".jpg")
        if args.save_images
        else None
    )
    ...
    for idx, sample in enumerate(loader):
        ...
        for jdx, classifier_scale in enumerate(classifier_scales):
            ...
            if "2" in args.method:
                ...
                mask, dil_mask = generate_mask(
                    img[~transformed, ...], cfs, args.dilation
                )
