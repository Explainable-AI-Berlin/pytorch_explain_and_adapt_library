...
def arguments():
    ...
    parser.add_argument(
        "--oracle-path",
        default="/scratch/ppar/models/oracle.pth",
        type=str,
        help="Oracle path",
    )
    parser.add_argument(
        "--exp-name", default="fastdime", type=str, help="Experiment Name"
    )
    parser.add_argument(
        "--output-path", default="/scratch/ppar/results", type=str, help="Results Path"
    )
...
if __name__ == '__main__':
    ...
    for i in range(40):
        corrs2[i] = np.corrcoef(df["Smiling"].to_numpy(), df.iloc[:, i + 1].to_numpy())[
            0, 1
        ]
    ...
    plot_bar([corrs, results], ["Correlation", "Method"], 40, sorted)
