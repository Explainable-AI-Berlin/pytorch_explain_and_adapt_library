...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="level of encoded correlation between the shortcut feature and the task label",
    )
    parser.add_argument(
        "--n_samples",
        type=int,
        default=5000,
        help="number of samples for each task label class",
    )
    parser.add_argument(
        "--data",
        default="/scratch/ppar/data/img_align_celeba",
        type=str,
        help="data path",
    )
    parser.add_argument(
        "--path", default="/scratch/ppar/results", type=str, help="results path"
    )
    parser.add_argument(
        "--exp-name", default="fastdime_shortcut", type=str, help="experiment name"
    )
...
def main():
    ...
    print(
        f"Balanced test_u and counterfactual testc_u test sets size: {len(balanced_testset)}\n"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for train_inputs, _, train_task_labels in train_Dk_loader:
            train_inputs, train_task_labels = train_inputs.to(
                device
            ), train_task_labels.to(device)
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            for inputs, _, task_labels in testk_loader:
                ...
                correct += (
                    (torch.round(predictions) == task_labels.float()).sum().item()
                )

        tesk_k_accuracy = correct / total
        test_k_aucroc = roc_auc_score(
            np.array(task_labels_list), np.array(predictions_list)
        )
        print(
            f"Epoch {epoch+1}/{num_epochs}, train loss: {loss:.4f}, test_k ACC: {tesk_k_accuracy:.2%}, test_k AUROC: {test_k_aucroc:.4f}"
        )
    ...
    with torch.no_grad():
        for original, counterfactual, task_labels, shortcut_labels in testu_loader:
            ...
            task_labels, shortcut_labels = task_labels.to(device), shortcut_labels.to(
                device
            )
            ...
            predictions_list_conterfactual.extend(
                predictions_counterfactual.cpu().numpy()
            )
    ...
    accuracy_original = np.mean(
        np.round(predictions_list_original) == task_labels_list_original
    )
    accuracy_counterfactual = np.mean(
        np.round(predictions_list_conterfactual) == task_labels_list_counterfactual
    )

    # calculate AUCROC separately for original set (test_u) and counterfactual set (testc_u)
    aucroc_original = roc_auc_score(
        task_labels_list_original, predictions_list_original
    )
    aucroc_counterfactual = roc_auc_score(
        task_labels_list_counterfactual, predictions_list_conterfactual
    )
    ...
    shortcut_cases_index = np.argwhere(
        np.array(shortcut_labels_list_counterfactual) == 1
    )
    non_shortcut_cases_index = np.argwhere(
        np.array(shortcut_labels_list_counterfactual) == 0
    )
    md_shortcut = np.mean(
        predictions_original[shortcut_cases_index]
        - predictions_counterfactual[shortcut_cases_index]
    )
    md_non_shortcut = np.mean(
        predictions_original[non_shortcut_cases_index]
        - predictions_counterfactual[non_shortcut_cases_index]
    )

    print(
        f"Results for dataset with k = {args.percentage * 100}% level of encoded correlation between the shortcut feature and the task label"
    )
