...
def merge_chunks(args):
    ...
    text_fn = lambda chunk: osp.join(
        args.output_path,
        "Results",
        args.exp_name,
        f"c-{chunk}_{args.chunks}-summary.yaml",
    )

    stats = {"cf": 0, "cf5": 0, "l1": 0, "l inf": 0, "p": 0}
    ...
    with open(
        osp.join(args.output_path, "Results", args.exp_name, "summary.yml"), "w"
    ) as f:
...
class ImageSaver:
    ...
    def save_img(
        self, img, cf, rec, idx, target, label, pred, pred_cf, l_inf, l_1, mask
    ):
...
def save_tokens_and_embeddings(
    ...
    token_embeds = (
        sd_model.text_encoder.get_input_embeddings().weight.data.detach().cpu()
    )
    placeholder_token_ids = sd_model.tokenizer.convert_tokens_to_ids(tokens)

    d = {"embs": token_embeds[placeholder_token_ids], "words": tokens}
    torch.save(d, output)
...
def load_tokens_and_embeddings(
    ...
    for file in files:
        ...
        assert num_added_tokens == len(
            d["words"]
        ), f"There are some tokens already from file {file} in Stable Diffusion"
        ...
        token_embeds[placeholder_token_ids] = d["embs"].to(
            token_embeds.device, dtype=token_embeds.dtype
        )


def add_new_tokens(new_tokens: list, init_tokens: list, sd_model, dtype=torch.float32):
    """
    ...
    assert num_added_tokens == len(
        new_tokens
    ), "No new token(s) added. Current model already has the proposed token(s)!"
    ...
    index_no_updates = torch.arange(len(sd_model.tokenizer)).view(
        -1, 1
