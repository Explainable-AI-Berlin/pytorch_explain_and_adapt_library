...
def generate_class_phrase(tokens, base_prompt, inbetween):
...
def get_phrase_generator(args):
    ...
    """
    if args.dataset == "CelebAHQ" or isinstance(args.dataset, torch.utils.data.Dataset):
        if args.phase == "context":
            return generate_context_phrase

        else:
            return partial(
                generate_class_phrase, base_prompt=args.base_prompt, inbetween="with a"
            )
