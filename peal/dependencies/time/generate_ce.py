...
def arguments():
    ...
    parser.add_argument(
        "--guidance-scale-denoising", type=float, default=[5], nargs="+"
    )
    parser.add_argument(
        "--guidance-scale-invertion", type=float, default=[5], nargs="+"
    )
...
def generate_time_counterfactuals(args=None):
    ...
    max_length = max(
        len(args.guidance_scale_invertion),
        len(args.guidance_scale_denoising),
        len(args.num_inference_steps),
    )
    ...
    for idx, (indexes, img, lab) in enumerate(loader):
        ...
        for jdx, (gsi, gsd, nis) in enumerate(
            zip(
                args.guidance_scale_invertion,
