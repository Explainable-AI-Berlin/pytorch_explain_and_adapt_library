...
from peal.architectures.basic_modules import TwoPathNetwork
from peal.architectures.basic_modules import PgelSequential
from peal.architectures.interfaces import InvertibleGenerator
from peal.utils import (
    requires_grad_,
    move_to_device
)
from peal.data.dataloaders import create_class_ordered_batch

class Logger:
    '''

    '''
    def __init__(self, config, model, optimizer, device, base_dir, criterions, val_dataloader, attacker = None, writer = None):
        '''
    ...
    def log_step(self, mode, pred, y, loss_logs):
        ...
        for criterion in loss_logs:
            self.writer.add_scalar(mode + '_' + criterion, loss_logs[criterion], self.config['training']['global_train_step'])

        self.losses.append(loss_logs['loss'])

        if len(set(['ce', 'bce']).intersection(self.config['task']['criterions'].keys())) >= 1:
            #
            ...
            self.correct.append((class_prediction == y.to(self.device)).type(torch.float))
    ...
    def log_epoch(self, mode):
        ...
        self.writer.add_scalar('epoch_' + mode + '_loss_accumulated', loss_accumulated.item(), self.config['training']['epoch'])
        print('')
        print('epoch_' + mode + '_loss_accumulated', str(loss_accumulated.item()))

        if 'ce' in self.config['task']['criterions'].keys():
            targets_one_hot = torch.nn.functional.one_hot(torch.cat(self.targets), self.output_size).to(torch.float32)
            predictions_one_hot = torch.nn.functional.one_hot(torch.cat(self.predicted_classes).to(torch.int64), self.output_size).to(torch.float32)

        if 'bce' in self.config['task']['criterions'].keys():
            ...
            self.writer.add_histogram('epoch_' + mode + '_correct_per_class', correct_per_class, self.config['training']['epoch'])

        if len(set(['ce', 'bce']).intersection(self.config['task']['criterions'].keys())) >= 1:
            accuracy = torch.cat(self.correct).mean().item()
            self.writer.add_scalar('epoch_' + mode + '_accuracy', accuracy, self.config['training']['epoch'])
            self.writer.add_histogram('epoch_' + mode + '_predicted_classes', predictions_one_hot.mean(0), self.config['training']['epoch'])
            self.writer.add_histogram('epoch_' + mode + '_targets', targets_one_hot.mean(0), self.config['training']['epoch'])
            ...
            print('epoch_' + mode + '_predicted_classes: ' + str(predictions_one_hot.mean(0)[:3]))
            print('epoch_' + mode + '_targets: ' + str(targets_one_hot.mean(0)[:3]))
            print('epoch_' + mode + '_classes_difference: ' + str(predictions_one_hot.mean(0).cpu()[:3] - targets_one_hot.mean(0).cpu()[:3]))
            #
            self.predictions = []
            self.targets = []
            self.predicted_classes = []
        ...
        if isinstance(self.model, InvertibleGenerator):
            reconstructed_images = self.model.decode(self.model.encode(self.test_X.to(self.device)))
            reconstruction_collage = torch.cat([reconstructed_images, self.test_X.to(self.device)])
            torchvision.utils.save_image(
                reconstruction_collage.cpu().data,
                os.path.join(self.base_dir, 'outputs', 'reconstruction_' + str(self.config['training']['epoch']) + '.png'),
                normalize=True,
                nrow=self.config['training']['val_batch_size'],
                range=(-0.5, 0.5),
            )
            if hasattr(self.val_dataloader.dataset, 'project_to_pytorch_default'):
                reconstruction_collage = self.val_dataloader.dataset.project_to_pytorch_default(reconstruction_collage)
            ...
            torchvision.utils.save_image(
                ...
                os.path.join(self.base_dir, 'outputs', 'samples_' + str(self.config['training']['epoch']) + '.png'),
                normalize=True,
                nrow=int(math.sqrt(self.config['training']['val_batch_size'])),
                range=(-0.5, 0.5),
            )
            if hasattr(self.val_dataloader.dataset, 'project_to_pytorch_default'):
                sample_images = self.val_dataloader.dataset.project_to_pytorch_default(sample_images)
