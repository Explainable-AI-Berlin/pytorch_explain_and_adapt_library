...
class AsyrpDDIMAdaptor(EditCapableGenerator):
    def __init__(self, config, dataset, model_dir=None, device="cpu"):
        ...
        if not hasattr(self.config, "data"):
            self.config.data = self.classifier_dataset.config.data
    ...
    def edit(
        ...
        '''dataset = Image2ClassDataset(
            ...
            mode=None,
            config=copy.deepcopy(self.classifier_dataset.config),
            transform=self.classifier_dataset.transform,
        )'''
        ...
        x_counterfactuals = self.classifier_dataset.project_from_pytorch_default(x_counterfactuals)
