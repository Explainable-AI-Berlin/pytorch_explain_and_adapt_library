    ...
    def __init__(self, config, encoder=None, decoder=None):
        ...
        if encoder is None:
            if self.config["data"]["input_type"] == "image":
                ...
                self.encoder = Sequence2LatentEncoder(self.config)
        ...
        if decoder is None:
            if self.config["data"]["input_type"] == "image":
                ...
                self.decoder = Latent2SequenceDecoder(self.config)
        ...
        self.prior = torch.distributions.Normal(
            torch.zeros(self.config["architecture"]["neuron_numbers_encoder"][-1]),
            torch.ones(self.config["architecture"]["neuron_numbers_encoder"][-1]),
        )
