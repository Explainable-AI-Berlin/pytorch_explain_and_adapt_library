...
class VirelayTeacher(TeacherInterface):
    ...
    def get_feedback(
        self, collage_paths, gt_classes, attributions, base_dir, source_classes, **args
    ):
        Path(base_dir).mkdir(parents=True, exist_ok=True)
        if not os.path.exists(os.path.join(base_dir, "database.hdf5")):
            collage_paths_dataloader = torch.utils.data.DataLoader(
                ImageDataset(os.path.split(collage_paths[0])[0])
            )
            create_dataset(
        ...
        if not os.path.exists(os.path.join(base_dir, "attribution_database.hdf5")):
            attributions_dataloader = list(
                zip(attributions, source_classes, gt_classes)
            )
