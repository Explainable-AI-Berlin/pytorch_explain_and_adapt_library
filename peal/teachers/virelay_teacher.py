...
from corelay.processor.embedding import EigenDecomposition, UMAPEmbedding, TSNEEmbedding
...
def create_dataset(dataset_file_path: str, samples_shape: tuple, number_of_samples: int, dataloader: torch.utils.data.DataLoader) -> h5py.File:
...
def create_attribution_database(
    ...
    attribution_database_file.create_dataset(
        'prediction',
        shape=(number_of_samples, num_classes),
        dtype='float32'
    )
    attribution_database_file.create_dataset(
        'label',
...
def create_attribution_dataset(dataloader, attribution_database_file_path, num_classes):
    ...
    with create_attribution_database(
            ...
            number_of_samples=number_of_samples) as attribution_database_file:
...
def meta_analysis(
    ...
    pipeline = SpectralClustering(
        ...
        clustering=Parallel([
            ...
            Parallel([
                ...
                #Parallel([UMAPEmbedding(n_neighbors=n_neigh, min_dist=min_dist) for n_neigh, min_dist in product([5, 10, 15, 20], [0.001, 0.1, 1.0, 10.0])], broadcast=True)
                Parallel([TSNEEmbedding(perplexity=float(perp), early_exaggeration=float(exagg)) for perp, exagg in product([5, 15, 30, 50, 100], [6, 12, 24])], broadcast=True)
            ], broadcast=True)
        ], broadcast=True, is_output=True)
    )
    ...
    if label_map_file_path is not None:
        ...
        wordnet_id_map = {label['index']: label['word_net_id']
                          for label in label_map}
        class_name_map = {label['index']: label['name'] for label in label_map}

    else:
        wordnet_id_map = {class_index: str(
            class_index) for class_index in class_indices}
        class_name_map = {class_index: str(
            class_index) for class_index in class_indices}
        label_map = [{'index': class_index, 'word_net_id': str(
            class_index), 'name': str(class_index)} for class_index in class_indices]
    ...
    for class_index in class_indices:
        ...
        with h5py.File(analysis_file_path, 'a') as analysis_file:
            ...
            analysis_name = wordnet_id_map.get(
                class_index, f'{class_index:08d}')
            ...
            analysis_group['index'] = indices_of_samples_in_class.astype(
                'uint32')
    ...
    with tqdm(range(100000)) as pbar:
        for it in pbar:
            if os.path.exists(output_path):
                ...
                pbar.set_description('Give feedback at localhost:' + str(port) +
                                     ' and save under ' + output_path + ' when done.')
                time.sleep(1.0)
...
class VirelayTeacher(TeacherInterface):
    def __init__(
        ...
        self.port = port
        self.teacher_config = load_yaml_config(teacher_config, return_namespace=False)

    def get_feedback(self, collage_paths, gt_classes, attributions, base_dir, source_classes, **args):
        Path(base_dir).mkdir(parents=True, exist_ok=True)
        if not os.path.exists(os.path.join(base_dir, 'database.hdf5')):
            collage_paths_dataloader = torch.utils.data.DataLoader(CollageDataset(os.path.split(collage_paths[0])[0]))
            create_dataset(
                dataset_file_path=os.path.join(base_dir, 'database.hdf5'),
                samples_shape=collage_paths_dataloader.dataset[0][0].shape,
                number_of_samples=len(collage_paths_dataloader.dataset),
                dataloader= collage_paths_dataloader,
            )
        ...
        if not os.path.exists(os.path.join(base_dir, 'attribution_database.hdf5')):
            attributions_dataloader = list(zip(attributions, source_classes, gt_classes))
            create_attribution_dataset(
                attribution_database_file_path=os.path.join(
                    base_dir,
                    'attribution_database.hdf5'
                ),
        ...
        feedback_out = []
        for i in range(len(collage_paths)):
