...
class SprayConfig(BaseModel):
    ...
    num_eigval: int = 32
    tsne_perplexity: Union[float, list[float]] = 64.0
...
class Spray(TeacherInterface):
    def __init__(self, config: Union[dict, SprayConfig], device: str = None):
        ...
        self.train_data, self.val_data, self.test_data = get_datasets(
            self.config.data, task_config=self.config.task, return_dict=True
        )
    ...
    def get_feedback(self, **args) -> dict[str, int]:
        ...
        layer_name = (
            "input"
            if self.attribution_layer == 0
            else get_layer_name(self.model, self.attribution_layer)
        )
        self.analysis_dir = os.path.join(
            self.base_dir, f"attrbs-layer-{self.attribution_layer}_analysis"
        )
        ...
        with open(
            os.path.join(self.analysis_dir, "data_spray_labels.csv"), "w", newline=""
        ) as f:
            csv.writer(f).writerows(new_data_file)

        if self.config.include_testsplit:
            dataloader = DataLoader(
                ConcatDataset([self.train_data, self.val_data, self.test_data]),
                batch_size=64,
                shuffle=False,
            )
        else:
            dataloader = DataLoader(
                ConcatDataset([self.train_data, self.val_data]),
                batch_size=64,
                shuffle=False,
            )
        ...
        for batch in dataloader:
            ...
            for i, filename in enumerate(batch["url"]):
                if filename in group_label_map:
                    ...
                    label_acc[targets[i].item()][spray_label].append(
                        true_labels[i].item() == spray_label
                    )
    ...
    def _compute_attributions(self, attribution_layer: str):
        ...
        self.concept_importance_db_path = os.path.join(
            self.analysis_dir, "concept_importance.h5"
        )
        ...
        if self.config.split_dataset:
            data = [
                (i, DataLoader(split, batch_size=64, shuffle=False))
                for i, split in enumerate(datasplits)
            ]
        else:
            data = [
                (
                    -1,
                    DataLoader(ConcatDataset(datasplits), batch_size=64, shuffle=False),
                )
            ]
        ...
        for split, dataloader in data:
            ...
            with tqdm(dataloader) as pbar:
                pbar.set_description(
                    f"computing relevances in dataset split {split} at layer {attribution_layer}..."
                )
                for batch in pbar:
                    ...
                    def one_hot_max(output):
                        ...
                        return (
                            logits[:, None]
                            * torch.eye(output.shape[1], device=self.device)[y]
                        )
                    ...
                    if concept_layer is None:
                        ...
                        condition = [
                            {
                                "y": [target],
                                concept_layer: self.config.conditioning_channels,
                            }
                            for target in y
                        ]

                    record_layers = (
                        [] if attribution_layer == "input" else [attribution_layer]
                    )
                    attr = attributor(
                        x, condition, composite, record_layer=record_layers
                    )
                    if self.config.skip_wrongly_classified_samples:
                        non_zero = (
                            (attr.heatmap.sum((1, 2)).abs().detach().cpu() > 0)
                            * (out.argmax(1).detach().cpu() == y)
                        ).numpy()
                    else:
                        non_zero = (
                            attr.heatmap.sum((1, 2)).abs().detach().cpu() > 0
                    ...
                    attributions = (
                        heatmaps
                        if attribution_layer == "input"
                        else attr.relevances[attribution_layer][non_zero]
                    )
                    # heatmaps = attributions.mean(dim=1)
                    heatmaps = heatmaps[:, None]
                    concept_importance = cc.attribute(attributions, abs_norm=True)[
                        :, None
                    ]
                    y = y[non_zero]
                    x = x[non_zero]
                    ...
                    if number_samples_processed == 0:
                        ...
                        attribution_db_file = create_attribution_database(
                            ...
                            self.config.classes_total,
                            len_data,
                        )
                        heatmaps_db_file = create_attribution_database(
                            self.heatmaps_db_path,
                            heatmaps[0].shape,
                            self.config.classes_total,
                            len_data,
                        )
                        concept_importance_db_file = create_attribution_database(
                            self.concept_importance_db_path,
                            concept_importance[0].shape,
                            self.config.classes_total,
                            len_data,
                        )
                        input_db_file = create_input_database(
                            self.input_db_path, x[0].shape, len_data
                        )

                    dataset_split = np.ones(len(y), dtype=np.uint16) * split
                    append_attributions(
                        attribution_db_file,
                        number_samples_processed,
                        attributions,
                        attr.prediction[non_zero],
                        y,
                        dataset_split,
                    )
                    append_attributions(
    ...
    def _spectral_clustering(self):
        ...
        pipeline = SpectralClustering(
            ...
            embedding=EigenDecomposition(
                n_eigval=self.config.num_eigval, is_output=True
            ),
            clustering=Parallel(
                [
                    Parallel(
                        [
                            KMeans(
                                n_clusters=number_of_clusters, kwargs={"max_iter": 700}
                            )
                    ...
                    Parallel(
                        [
                            TSNEEmbedding(
                                ...
                                kwargs={"learning_rate": "auto", "init": "pca"},
                            )
                            for perplexity in self.config.tsne_perplexity
                        ],
                        broadcast=True,
                    ),
                    Parallel(
                        [
                            UMAPEmbedding(n_neighbors=n_neighbors)
                            for n_neighbors in self.config.umap_neighbors
                        ],
                        broadcast=True,
                    ),
                ],
        ...
        attribution_path = (
            self.concept_importance_db_path
            if self.config.use_relative_concept_importance
            else self.attribution_db_path
        )
        ...
        for split, split_name in split_names:
            for class_idx in range(self.config.classes_total):
                ...
                with h5py.File(analysis_db_path, "a") as analysis_file:
                    ...
                    analysis_name = (
                        CLASS_NAMES[self.dataset_class][class_idx] + f" ({split_name})"
                    )
                    ...
                    embedding_group["spectral"].attrs["eigenvalue"] = (
                        eigenvalues.astype(np.float32)
                    )

                    for perplexity, tsne_embedding in zip(
                        self.config.tsne_perplexity, tsne
                    ):
                        embedding_name = f"tsne-{perplexity:0>5.1f}"
                        embedding_group[embedding_name] = tsne_embedding.astype(
                            np.float32
                        )
                        embedding_group[embedding_name].attrs["embedding"] = "spectral"
                        embedding_group[embedding_name].attrs["index"] = np.array(
                            [0, 1]
                        )

                    for n_neighbors, umap_embedding in zip(
                        self.config.umap_neighbors, umap
                    ):
                        embedding_name = f"umap-{n_neighbors:0>3d}"
                        embedding_group[embedding_name] = umap_embedding.astype(
                            np.float32
                        )
                        embedding_group[embedding_name].attrs["embedding"] = "spectral"
                        embedding_group[embedding_name].attrs["index"] = np.array(
                    ...
                    for number_of_clusters, clustering in zip(
                        number_of_clusters_list, kmeans
                    ):
                        clustering_dataset_name = f"kmeans-{number_of_clusters:02d}"
                        cluster_group[clustering_dataset_name] = clustering
                        cluster_group[clustering_dataset_name].attrs[
                            "embedding"
                        ] = "spectral"
                        cluster_group[clustering_dataset_name].attrs[
                            "k"
                        ] = number_of_clusters
                        cluster_group[clustering_dataset_name].attrs["index"] = (
                            np.arange(embedding.shape[1], dtype=np.uint32)
                        )

                    for number_of_clusters, clustering in zip(
                        number_of_clusters_list, ac
                    ):
                        clustering_dataset_name = (
                            f"AgglomerativeClustering-{number_of_clusters:02d}"
                        )
                        cluster_group[clustering_dataset_name] = clustering
                        cluster_group[clustering_dataset_name].attrs[
                            "embedding"
    ...
    def _create_label_map(self) -> str:
        ...
        label_map = [
            {"index": idx, "name": class_names[idx], "word_net_id": str(idx)}
    ...
    def _run_virelay(self) -> tuple[list[int], list[int]]:
        ...
        for file in os.listdir(group_label_dir):
            ...
            if "non" in file:
                non_confounder_ids.extend(
                    json.load(open(file, "r"))["selectedDataPointIndices"]
                )
            else:
                confounder_ids.extend(
                    json.load(open(file, "r"))["selectedDataPointIndices"]
                )
...
def create_input_database(dataset_file_path, samples_shape, number_of_samples):
    ...
    dataset_file.create_dataset(
        "label", shape=(number_of_samples,), maxshape=(None,), dtype="uint16"
    )
    dataset_file.create_dataset(
        "filenames",
...
def create_attribution_database(
    ...
    attribution_database_file.create_dataset(
        "dataset_split", shape=(number_of_samples,), maxshape=(None,), dtype="int16"
    )
...
def append_attributions(
    ...
    attribution_database_file["dataset_split"][
        index : dataset_split.shape[0] + index
    ] = dataset_split
