...
class SimpleProjection(torch.nn.Module):
    ...
    def __init__(self, uncorrect_decision_strategy, correct_decision_strategy):
        ...
        '''
        super().__init__()
        self.difference = torch.stack(uncorrect_decision_strategy).mean(0) - torch.stack(correct_decision_strategy).mean(0)
...
class SVMProjection(torch.nn.Module):
    def __init__(self, uncorrect_decision_strategy, correct_decision_strategy, adaptor_config):
        super().__init__()
        ...
        features = torch.cat([uncorrect_decision_strategy, correct_decision_strategy], 0).cpu() #features[valid_indices]
        artifacts = torch.cat(
            [torch.ones([uncorrect_decision_strategy.shape[0]]), torch.zeros([correct_decision_strategy.shape[0]])]
        ).cpu()
        self.model = LinearSVC(C=1000.0, penalty='l2', max_iter=10000, class_weight='balanced', verbose = 2)
        ...
        # w is not normalized
        svm_norm = ((svm_cav ** 2).sum() ** .5)
        #svm_cav = svm_cav / svm_norm
        self.cav = torch.tensor(self.adaptor_config['correction_strength'] * svm_cav / svm_norm)
    ...
    def forward(self, x):
        ...
        x_flat = x.flatten(1)
        return (x_flat - x_flat @ self.cav[:, None].to(x) @ self.cav[None].to(x) + self.bias[None].to(x) @ self.cav[:, None].to(x) @ self.cav[None].to(x)).reshape(x.shape)
...
class ClassArtifactCompensation:
    def __init__(
        ...
        if isinstance(teacher, TeacherInterface):
            ...
            teacher.eval()
            teacher_explainer = LRPExplainer(teacher, self.output_size, self.adaptor_config['explainer'])
            self.teacher = HeatmapComparisonTeacher(teacher_explainer, self.adaptor_config['attribution_threshold'])
            ...
            self.teacher = SegmentationMaskTeacher(self.adaptor_config['attribution_threshold'])
    ...
    def generate_lrp_heatmaps(self, dataloader, collage_dir, max_samples = 10000000):
        ...
        for idx in range(len(images)):
            ...
            collage_path = os.path.join(collage_dir, embed_numberstring(str(idx)) + '.png')
    ...
    def calculate_validation_statistics(self, finetune_iteration):
        collage_paths, heatmaps, gt_classes, hints, attributions, images, predictions = self.generate_lrp_heatmaps(
            self.val_dataloader,
        ...
        accuracy = calculate_test_accuracy(self.student, self.val_dataloader, self.device)
    ...
    def run(self):
        ...
        if not os.path.exists(os.path.join(self.base_dir, '0')):
            ...
            if self.output_size == 2: self.visualize_progress([os.path.join(self.base_dir, 'visualization.png')])
        ...
        for finetune_iteration in range(1, self.adaptor_config['finetune_iterations'] + 1):
            if not os.path.exists(os.path.join(self.base_dir, str(finetune_iteration))):
                ...
                os.makedirs(os.path.join(self.base_dir, str(finetune_iteration), 'collages'))
                os.makedirs(os.path.join(self.base_dir, str(finetune_iteration), 'validation_collages'))
                collage_paths, heatmaps, gt_classes, hints, attributions, images, predictions = self.generate_lrp_heatmaps(
                    dataloader = self.train_dataloader,
                    collage_dir = os.path.join(self.base_dir, str(finetune_iteration), 'collages'),
                ...
                collage_paths = []
                files = os.listdir(os.path.join(self.base_dir, str(finetune_iteration), 'collages'))
                files.sort()
                for file in files:
                    collage_paths.append(os.path.join(self.base_dir, str(finetune_iteration), 'collages', file))
            ...
            if self.adaptor_config['projection_location'] == 'features':
                ...
                correct_decision_strategy = [x for x, fb, l in zip(images, feedback, labels) if fb == 'true' and l == 1]
                uncorrect_decision_strategy = [x for x, fb, l in zip(images, feedback, labels) if fb == 'false' and l == 1]
            
            if self.adaptor_config['projection_type'] == 'simple':
                projection = SimpleProjection(uncorrect_decision_strategy, correct_decision_strategy)
            
            elif self.adaptor_config['projection_type'] == 'svm':
                projection = SVMProjection(uncorrect_decision_strategy, correct_decision_strategy, self.adaptor_config)
            ...
            if self.output_size == 2: self.visualize_progress([
                os.path.join(self.base_dir, str(finetune_iteration), 'visualization.png'),
                os.path.join(self.base_dir, 'visualization.png')
            ])

            torch.save(self.student, os.path.join(self.base_dir, str(finetune_iteration), 'model.cpl'))
