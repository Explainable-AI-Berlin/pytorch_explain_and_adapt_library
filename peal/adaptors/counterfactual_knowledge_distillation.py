...
class CounterfactualKnowledgeDistillation:
    def __init__(
        ...
        if isinstance(datasource[0], torch.utils.data.Dataset):
            ...
            if hasattr(datasource[0], "config"):
                self.adaptor_config["data"] = datasource[0].config
        ...
        self.use_visualization = (
            use_visualization and self.adaptor_config["data"]["input_type"] == "image"
        )
        ...
        for batch_idx in range(num_batches):
            ...
            with tqdm(range(100 * self.train_dataloader.dataset.__len__())) as pbar:
                for i in pbar:
                    ...
                    pbar.set_description(
                        "Sample selection phase: num_samples: "
                        + str(
                            self.adaptor_config["samples_per_iteration"]
                            - num_samples
                            + len(collage_paths)
            ...
            for sample_idx in range(result_img_collage.shape[0]):
                if (
                    ...
                    title_string = (
                        str(source_classes_current_batch[sample_idx])
                        + " -> "
                        + str(target_classes_current_batch[sample_idx].item())
                    )
                    title_string += (
                        ", Target: "
                        + str(round(float(start_target_confidences[sample_idx]), 2))
                        + " -> "
                        + str(round(float(end_target_confidences[sample_idx]), 2))
                    )
                    ...
                    '''img = img.resize(
                        (
                            3 * self.adaptor_config["data"]["input_size"][1],
                            3 * self.adaptor_config["data"]["input_size"][1],
                        )
                    )'''
                    img.save(collage_path)
                    counterfactuals.append(
                        self.project_to_pytorch_default(counterfactual[sample_idx])
                    )
                    heatmaps.append(
                        self.project_to_pytorch_default(
                            heatmaps_current_batch[sample_idx]
                        )
                    )
                    collage_paths.append(collage_path)
                    source_classes.append(source_classes_current_batch[sample_idx])
                    target_classes.append(
                        target_classes_current_batch[sample_idx].item()
                    )
    ...
    def generate_counterfactuals(
        self, error_distribution, confidence_score_stats, finetune_iteration
    ):
        if isinstance(self.teacher, SegmentationMaskTeacher):
    ...
    def calculate_validation_statistics(self, finetune_iteration):
        ...
        with tqdm(enumerate(self.dataloaders_val[0])) as pbar:
            for it, (X, y) in pbar:
                pred_confidences = (
                    torch.nn.Softmax()(self.student(X.to(self.device))).detach().cpu()
                )
                ...
                start_target_confidences.append(
        ...
        if self.adaptor_config["use_confusion_matrix"] and not accuracy == 1.0:
            ...
            error_matrix = torch.ones([self.output_size, self.output_size]) - torch.eye(
                self.output_size
            )
    ...
    def retrieve_validation_statistics(self, finetune_iteration):
        ...
        if not os.path.exists(
            os.path.join(
                self.base_dir, str(finetune_iteration), "validation_counterfactuals.npz"
            )
        ):
            validation_basics, validation_lists = self.calculate_validation_statistics(
                finetune_iteration
            )
            ...
            with open(
                os.path.join(
                    self.base_dir, str(finetune_iteration), "validation_basics.npz"
                ),
                "rb",
            ) as f:
            ...
            with open(
                os.path.join(
                    self.base_dir,
                    str(finetune_iteration),
                    "validation_counterfactuals.npz",
                ),
                "rb",
            ) as f:
                ...
                y_pred_list = list(
                    torch.tensor(validation_counterfactuals["y_pred_list"])
                )
                counterfactual_list = list(
                    torch.tensor(validation_counterfactuals["counterfactual_list"])
                )
                heatmap_list = list(
                    torch.tensor(validation_counterfactuals["heatmap_list"])
                )
                target_class_list = list(
                    torch.tensor(validation_counterfactuals["target_class_list"])
                )
                hint_list = list(torch.tensor(validation_counterfactuals["hint_list"]))
                attribution_list = list(
                    torch.tensor(validation_counterfactuals["attribution_list"])
                )
        ...
        if not os.path.exists(
            os.path.join(
                self.base_dir, str(finetune_iteration), "feedback_validation.txt"
            )
        ):
            feedback = self.teacher.get_feedback(
                ...
                base_dir=os.path.join(
                    self.base_dir, str(finetune_iteration), "validation_teacher"
                ),
            )
        ...
        num_true_1sided = len(
            list(
                filter(
                    lambda x: x[1] == "true" and y_list[x[0]] == y_pred_list[x[0]],
                    enumerate(feedback),
                )
            )
        )
        num_false_1sided = len(
            list(
                filter(
                    lambda x: x[1] == "false" and y_list[x[0]] == y_pred_list[x[0]],
                    enumerate(feedback),
                )
            )
        )
    ...
    def run(self):
        ...
        """
        if self.overwrite or not os.path.exists(
            os.path.join(self.base_dir, "0", "validation_counterfactuals.npz")
        ):
            ...
            (
                ...
                fa_1sided,
                fa_absolute,
            ) = self.retrieve_validation_statistics(0)
            ...
            (
                ...
                fa_1sided,
                fa_absolute,
            ) = self.retrieve_validation_statistics(
        ...
        writer.add_scalar(
            "ood_rate", ood_rate, self.adaptor_config["current_iteration"]
        )
        writer.add_scalar(
            "fa_2sided", fa_2sided, self.adaptor_config["current_iteration"]
        )
        writer.add_scalar(
            "fa_1sided", fa_1sided, self.adaptor_config["current_iteration"]
        )
        writer.add_scalar(
            "fa_absolute", fa_absolute, self.adaptor_config["current_iteration"]
        )
        ...
        for finetune_iteration in range(
            ...
            current_dataset_path = os.path.join(
                self.base_dir, str(finetune_iteration), "dataset"
            )
            ...
            if not os.path.exists(
                ...
                with open(
                    ...
                    counterfactuals = list(
                        torch.tensor(validation_counterfactuals["counterfactuals"])
                    )
                    heatmaps = list(
                        torch.tensor(validation_counterfactuals["heatmaps"])
                    )
                    source_classes = list(
                        torch.tensor(validation_counterfactuals["source_classes"])
                    )
                    target_classes = list(
                        torch.tensor(validation_counterfactuals["target_classes"])
                    )
                    hints = list(torch.tensor(validation_counterfactuals["hints"]))
                    attributions = list(
                        torch.tensor(validation_counterfactuals["attributions"])
                    )
            ...
            if not os.path.exists(
                os.path.join(self.base_dir, str(finetune_iteration), "feedback.txt")
            ):
                feedback = self.teacher.get_feedback(
                ...
                with open(
                    os.path.join(
                        self.base_dir, str(finetune_iteration), "feedback.txt"
                    ),
                    "w",
                ) as f:
                    f.write("\n".join(feedback))
            ...
            (
                ...
                fa_1sided,
                fa_absolute,
            ) = self.retrieve_validation_statistics(finetune_iteration)
            writer.add_scalar("val_accuracy", val_accuracy, finetune_iteration)
            writer.add_scalar(
                "counterfactual_rate", counterfactual_rate, finetune_iteration
            )
...
def create_dataset(
    ...
    if isinstance(dataset, SymbolicDataset):
        ...
        positive_samples = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] == "true", zip(counterfactuals, feedback)),
            )
        )
        if len(positive_samples) > 0:
            positive_samples = torch.stack(positive_samples)
            positive_samples = torch.cat(
                [positive_samples, torch.ones([positive_samples.shape[0], 1])], dim=1
            )
        ...
        negative_samples = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] == "false", zip(counterfactuals, feedback)),
            )
        )
        if len(negative_samples) > 0:
            negative_samples = torch.stack(negative_samples)
            negative_samples = torch.cat(
                [negative_samples, torch.zeros([negative_samples.shape[0], 1])], dim=1
            )
            samples.append(positive_samples)
        ...
        np.savetxt(
            ...
            samples.numpy(),
            delimiter=",",
            header=",".join(dataset.attributes),
        )
        ...
        if isinstance(teacher, SegmentationMaskTeacher):
            ...
            Path(
                os.path.join(
                    base_dir, str(finetune_iteration), mode + "dataset", "masks"
                )
            ).mkdir(parents=True, exist_ok=True)
        ...
        for class_name in range(output_size):
            Path(
                os.path.join(
                    base_dir, str(finetune_iteration), img_dir, str(class_name)
                )
            ).mkdir(parents=True, exist_ok=True)
