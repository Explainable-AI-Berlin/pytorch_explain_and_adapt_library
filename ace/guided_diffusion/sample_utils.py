    ...
    def p_sample_loop(
        ...
        for jdx, i in enumerate(indices):
            ...
            if (class_grad_fn is not None) and (guided_iterations > jdx):
                if jdx == 0:
                    grads = (
                        grads + class_grad_fn(x_t=x_t, **class_grad_kwargs) / alpha_t
                    )
                else:
                    with torch.enable_grad():
                        z_in = z_t.detach().requires_grad_(True)
                        loss, x_t = full_class_grads(
                            diffusion=diffusion,
                            model=model,
                            shape=shape,
...
def mask_opt_fn(
    ...
    for i in range(iters):
        ...
        with torch.no_grad():
            l_mask_grads = -torch.ones_like(mask) * (mask > topk) + torch.ones_like(
                mask
            ) * (mask <= topk)
            ...
            mask = update_mask_fn(
                mask, mask_grads + l_mask_grads + tv_grad, **update_mask_kwargs
            )
...
@torch.enable_grad()
def mask_opt_fn2(x_tau, x_cf, y, classifier, top_k, use_logits, multiclass=False):
    classifier.eval()
    if multiclass:
    ...
    def classification_loss(x_tau, x_cf, mask, t):
        ...
        if not multiclass:
            ...
            selected = (
                -logits[range(logits.size(0)), t]
                if use_logits
                else criterion(logits, t)
            )
    ...
    while True:
        ...
        l_class_ce = classification_loss(
            x_tau[~changed], x_cf[~changed], big_mask, y[~changed]
        )
        l_class_ta = classification_loss(
            x_cf[~changed], x_tau[~changed], big_mask, lab[~changed]
        )

        # gradients and select the pixel with largest magnitud
        mask_grads = torch.autograd.grad(
            l_class_ce + l_class_ta, mask, allow_unused=True
        )[0]
        ...
        with torch.no_grad():
            ...
            if top_k != -1:
                ...
                changed[~changed] = predict_on_mask(
                    x_tau[~changed], x_cf[~changed], mask[~changed], y[~changed]
                )
...
@torch.enable_grad()
def mask_opt_fn3(x_tau, x_cf, y, classifier, top_k, use_logits, multiclass=False):
    ...
    dummy_mask = torch.zeros(
        *mask_shape, dtype=x_tau.dtype, device=x_tau.device, requires_grad=True
    )

    loss = classification_loss(
        x_cf, +x_tau * (1 - dummy_mask), y
    ...
    with torch.no_grad():
        ...
        seg_grads = torch.ones(
            mask_shape[0], num_segments, device=x_tau.device
        ) * float("inf")
        ...
        jdxs = indexes + torch.arange(
            0, sort_grads.size(0), device=indexes.device
        ).view(-1, 1) * sort_grads.size(1)
        jdxs = jdxs.view(-1)
...
def get_masked_DiME_iterative_sampling(use_sampling=False):
    ...
    def p_sample_loop(
        ...
        if mask_opt_fn is not None:
            mask = torch.ones(
                img.size(0), 1, img.size(2), img.size(3), device=img.device
            )
        ...
        for jdx, i in enumerate(indices):
            ...
            if (class_grad_fn is not None) and (guided_iterations > jdx):
                ...
                grads = (
                    grads
                    + class_grad_fn(
                        x_t=bin_mask * x_t + (1 - bin_mask) * img, **class_grad_kwargs
                    )
                    / alpha_t
                )
            ...
            if mask_opt_fn is not None:
                mask = mask_opt_fn(mask, img, x_t, **mask_opt_kwargs)
...
def logits_mask_opt_fn(
    ...
    for i in range(iters):
        ...
        with torch.no_grad():
            ...
            # update the initial mask
            log_mask = update_mask_fn(
                log_mask, mask_grads + l_mask_grads + tv_grad, **update_mask_kwargs
            )
...
def get_masked_logit_DiME_iterative_sampling(use_sampling=False):
    ...
    def p_sample_loop(
        ...
        log_mask = (
            6 * torch.ones(img.size(0), 1, img.size(2), img.size(3), device=img.device)
            if mask_opt_fn is not None
            else None
        )
        ...
        for jdx, i in enumerate(indices):
            ...
            # update the mask if possible
            if mask_opt_fn is not None:
                log_mask = mask_opt_fn(log_mask, img, x_t, **mask_opt_kwargs)
...
def get_static_mask(
    ...
    if mask_type == 0:
        ...
        mask = mask_opt_fn(x, x_ce, y, classifier, **mask_opt_kwargs)
...
def get_static_masked_DiME_iterative_sampling(use_sampling=False):
    ...
    def p_sample_loop(
        ...
        if mask_opt_kwargs is not None:
            mask = get_static_mask(
                img,
                class_grad_kwargs["classifier"],
                class_grad_kwargs["y"],
                None,
                mask_opt_fn,
                mask_opt_kwargs,
        ...
        for jdx, i in enumerate(indices):
            ...
            if (class_grad_fn is not None) and (guided_iterations > jdx):
                grads = (
                    grads
                    + class_grad_fn(
                        x_t=mask * x_t + (1 - mask) * img, **class_grad_kwargs
                    )
                    / alpha_t
                )
                grads = grads * mask
...
class ImageSaver:
    ...
    def save_img(
        self, img, cf, noise, idx, target, label, pred, pred_cf, l_inf, l_1, mask
    ):
