...
class OracleResnet(torch.nn.Module):
    def __init__(
        self,
        weights_path,
        num_classes=40,
        ...
        super().__init__()
        self.feat_extract = resnet50_ft_dag(weights_path)
        self.feat_extract.classifier = Identity()
        self.feat_extract.output_size = 2048
        self.classifier = torch.nn.Linear(self.feat_extract.output_size, num_classes)
        if freeze_layers:
            for param in self.feat_extract.parameters():
                param.requires_grad = False

            if unfreeze_last_block:
                for param in self.feat_extract._modules:
                    if param.find("conv5") != -1:
                        # print(param)
                        getattr(self.feat_extract, param).requires_grad = True

    def forward(self, x):
        feat_extract = self.feat_extract(x)[0]
        scores = self.classifier(feat_extract)
