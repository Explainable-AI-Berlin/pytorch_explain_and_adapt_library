...
class TestMakeImageGrid(unittest.TestCase):
    def test_sample1(self):
        ...
        green_img[:, :, 1] = 1.0
        tensor_one = torch.tensor(np.stack([blue_img, red_img, green_img], axis=0).transpose(0, 3, 1, 2))
        tensor_two = torch.tensor(np.stack([red_img, green_img, blue_img], axis=0).transpose(0, 3, 1, 2))
        tensor_three = torch.tensor(np.stack([green_img, blue_img, red_img], axis=0).transpose(0, 3, 1, 2))
        images = {'tensor1': [tensor_one, ['a', 'b', 'c']], 'tensor2': [[tensor_two, tensor_three], ['d', 'e', 'f']]}
