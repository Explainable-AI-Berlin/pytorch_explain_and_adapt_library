...
class CFDataset:
    def __init__(self, path, exp_name_format, values):
        ...
        for CL, CF in itertools.product(["CC", "IC"], ["CCF", "ICF"]):
            ...
            files = [
                os.listdir(osp.join(path, "Results", en, CL, CF, "CF"))
                for en in self.exp_names
            ]
...
@torch.no_grad()
def compute_LPIPS(LPIPS, path, exp_name_format, values, device):
    ...
    for cfs in tqdm(loader):
        ...
        for i in range(len(values)):
            ...
            for j in range(i + 1, len(values)):
                ...
                dist.append(
                    LPIPS.forward(cf1, cf2, normalize=False).squeeze()
...
if __name__ == "__main__":
    ...
    res = compute_LPIPS(
        LPIPS, args.output_path, args.exp_pattern, args.exp_values, device
    )
